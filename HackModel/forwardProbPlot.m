function [smrPlot, falPlot, wtrPlot] = forwardProbPlot(data, forward_prob, Sn, Hn, Wn)
    % check valid inputs
    if Sn ~= 3
    ...
    if size(forward_prob, 1) ~= datasize
        error('Length of forward probabilities must match length of data.');
    end
    ...
    for i = 1 : datasize
        switch (data(i, 2))
            case 0
                switch (data(i, 4))
                    case 0
                        smrWend(data(i, 3) + 1) = smrWend(data(i, 3) + 1) + forward_prob(i, 2);
                        smrWendN(data(i, 3) + 1) = smrWendN(data(i, 3) + 1) + 1;
                    case 1
                        smrWday(data(i, 3) + 1) = smrWday(data(i, 3) + 1) + forward_prob(i, 2);
                        smrWdayN(data(i, 3) + 1) = smrWdayN(data(i, 3) + 1) + 1;
                end
            case 1
                switch (data(i, 4))
                    case 0
                        falWend(data(i, 3) + 1) = falWend(data(i, 3) + 1) + forward_prob(i, 2);
                        falWendN(data(i, 3) + 1) = falWendN(data(i, 3) + 1) + 1;
                    case 1
                        falWday(data(i, 3) + 1) = falWday(data(i, 3) + 1) + forward_prob(i, 2);
                        falWdayN(data(i, 3) + 1) = falWdayN(data(i, 3) + 1) + 1;
                end
            case 2
                switch (data(i, 4))
                    case 0
                        wtrWend(data(i, 3) + 1) = wtrWend(data(i, 3) + 1) + forward_prob(i, 2);
                        wtrWendN(data(i, 3) + 1) = wtrWendN(data(i, 3) + 1) + 1;
                    case 1
                        wtrWday(data(i, 3) + 1) = wtrWday(data(i, 3) + 1) + forward_prob(i, 2);
